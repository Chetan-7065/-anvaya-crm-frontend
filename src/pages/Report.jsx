...
export default function Report() {
  ...
  const {data: leadData , loading: leadLoading, error: leadError} = useFetch("https://anvaya-crm-backend-puce.vercel.app/leads")
 ...
 const options = {
  ...
  const processedData = useMemo(() => {
    const counts = {};
    leadsData.length > 0 && leadsData.forEach((leads) => {
      counts[leads.salesAgent.name] = (counts[leads.salesAgent.name] || 0) + 1;
    });
    return counts;
  }, [leadsData]);
  ...
  const barOptions = {
    ...
    scales: {
      y: {
        ...
        title: { display: true, text: "Number of Leads", font: {
            size: 14,
            weight: "600",
          }  },
        grid: {
      ...
      x: {
        title: { display: true, text: "Sales Agents" , font: {
            size: 14,
            weight: "600",
          }  },
        ticks: {
  ...
  const statusData = useMemo(() => {
    const counts = {};
   leadsData.length > 0 && leadsData.forEach((lead) => {
      counts[lead.status] =
